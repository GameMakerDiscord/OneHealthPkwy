/// @desc Tweening Helper function for tweening an object's position
...
with (targetId) {
	if (sx != dx)
		x = script_execute(easing, currentTime, sx, dx, duration);
	if (sy != dy)
		y = script_execute(easingY, currentTime, sy, dy, duration);
}
